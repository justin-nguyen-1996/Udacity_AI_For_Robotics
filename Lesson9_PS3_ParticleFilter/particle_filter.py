...
class robot:
    ...
    def move(self, motion):
        ...
        a = random.gauss( motion[0], res.steering_noise )
        d = random.gauss( motion[1], distance_noise )
        ...
        res.x = cx + sin(self.orientation+b) * r
        res.y = cy - cos(self.orientation+b) * r
        res.orientation = (self.orientation+b) % (2*pi)
    ...
    def sense(self, is_noisy):
        Z = []
        for landmark in landmarks:
            delta_y = landmark[0] - self.y
            delta_x = landmark[1] - self.x
            bearing = (atan2(delta_y,delta_x) - self.orientation) % (2*pi)
            if is_noisy:
